...
function SearchBar({ onSearch }) {
  ...
  const handleFormSubmit = async (event) => {
    ...
    const response = await axios.get(
      `https://tarea-1.2023-1.tallerdeintegracion.cl/search/trays?name=${searchQuery}`
    );
...
function MenuList() {
  ...
  useEffect(() => {
    const fetchMenus = async () => {
      const response = await axios.get(
        `https://tarea-1.2023-1.tallerdeintegracion.cl/trays?sort=${sort}&order=${order ? "asc" : "desc"}&page=${currentPage}&size=${pageSize}`
      );
      ...
      menus.map((menu) => (
        <Col xs={12} sm={6} md={4} lg={3} key={menu.id}>
          <div className="card mb-4">
            <div className="card-img-top restaurant-image"></div>
            <div className="card-body">
              <h3 className="card-title text-center">{menu.name}</h3>
              <p className="card-text">{menu.description}</p>
              <div className="menu-item-price text-center">${menu.price}</div>
              <Link to={`/menus/${menu.id}`} className="btn btn-primary btn-block mt-3">Ver detalle</Link>
            </div>
          </div>
        </Col>
      ))}
